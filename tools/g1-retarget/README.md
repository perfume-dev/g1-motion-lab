...
used by [G1 Motion Lab's three viewers](../../README.md).
...
layout. Solver and parser code is preserved; the exporter's source-location
metadata is adapted to this standalone repository, retaining upstream origin.
[SOURCE_SNAPSHOT.json](SOURCE_SNAPSHOT.json) records current SHA-256 values.
The complete A/B/C recordings were validated with
...
From this standalone repository on that workstation:
...
`data/source`, with 1,300 frames and a 0.025-second interval (40 Hz).
...
python preflight/bvh_audit.py \
  ...
  --of-data "$G1_REPO_ROOT/data/source" \
...
g1_a() {
  python retarget/global_path.py \
    ...
    --source "$G1_REPO_ROOT/data/source/A_test.bvh" \
...
python retarget/retarget.py \
  --model models/unitree_g1 \
  --source "$G1_REPO_ROOT/data/source" \
